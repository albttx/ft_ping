...
/*   Updated: 2017/02/27 12:50:46 by ale-batt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			pinger(int signal)
{
	...
	icmp_packet->icmp_cksum = in_cksum((unsigned short *)icmp_packet, env.datalen + 8);
	gettimeofday(&(env.send_time), NULL);
	cc = sendto(env.sock, packet, env.datalen + 8, 0, (struct sockaddr *)&sin, sizeof(struct sockaddr_in));
	if (!cc)
	{
		perror("sendto");
		return ;
